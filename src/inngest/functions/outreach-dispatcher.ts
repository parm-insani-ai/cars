...
// Walks running outreach campaigns every minute and places the next batch of
// outbound AI sales calls, respecting per-campaign quiet hours (in each
// prospect's own timezone), rate per minute, max attempts, and the do-not-call
// suppression list. When the outreach Vapi assistant isn't configured, targets
// are marked "skipped" with a clear note so the UI never lies about progress.
  ...
  async () => {
    ...
    for (const c of campaigns) {
      ...
      for (const t of targets) {
        ...
        // Compliance gate 1: prospect-level do-not-call flag.
        if (p.doNotCall) {
          await skip(t.id, "opted_out", "Prospect is on the do-not-call list");
          totalSkipped++;
          continue;
        }
        // Compliance gate 2: number-level suppression list.
        if (p.phone) {
        ...
        // Quiet hours, evaluated in the prospect's own timezone.
        if (isQuietHour(p.timezone, c.quietStartHour, c.quietEndHour)) {
          continue; // leave pending — we'll pick it up in a later, allowed window
        }
...
function isQuietHour(tz: string, startHour: number, endHour: number): boolean {
  ...
  return h >= startHour || h < endHour; // wraps midnight, e.g. 20–9
}
